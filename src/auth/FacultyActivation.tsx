...
import { Loader2, Lock, GraduationCap, ArrowRight } from "lucide-react";
...
export default function FacultyActivation() {
  ...
  useEffect(() => {
    // When landing on this page via an invitation link, Supabase will have updated the session
  ...
  const onSubmit = async (data: ActivationFormValues) => {
    ...
    try {
      // The user is already logged in (via the invite link token), 
      // so we just update their password to the permanent one.
      ...
      toast({
        ...
        description: "Your faculty account is now active. Please log in with your new password.",
      });

      // Sign them out so they are forced to log in with the new password
      ...
      toast({
        ...
        description: error.message || "Could not set your permanent password.",
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-md relative">
        ...
        >
          ...
          <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">
            Welcome, {facultyName || "Professor"}!
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-8">
            You have been invited to join the ProjectHub evaluation committee. Please set a secure password to activate your account.
          </p>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase">Create Password</label>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex items-center justify-center py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-md transition-all disabled:opacity-70 disabled:cursor-not-allowed group"
            >
              {isLoading ? (
                <>
                ...
                <>
                  Activate Account <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>
          </form>
