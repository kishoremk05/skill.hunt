  ...
  return (
    <div className="border-b border-black/18 py-5">
      <button
        ...
        className="w-full flex justify-between items-center text-left py-2 text-[#1a1a1a] hover:opacity-60 transition-opacity duration-200"
      >
        <span className="text-base sm:text-[16px] uppercase font-bold tracking-wide">{question}</span>
        <ChevronDown
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className="pt-2 pb-4 text-[13px] text-[#3a3a3a] leading-relaxed max-w-4xl font-medium">
              {answer}
            </p>
          </motion.div>
        )}
...
export default function FAQSection() {
  const faqs = [
    ...
    },
    {
      ...
      answer: "The final score is a weighted combination of Faculty and Student evaluation. By default, the Faculty score accounts for 80% of the rating, while Peer Votes account for the remaining 20%.",
    },
    {
  ...
  return (
    <section className="relative z-[2] bg-transparent text-[#1a1a1a] flex flex-col px-8 md:px-[32px] py-[80px] max-md:px-[18px] max-md:py-[32px]">
      <div className="max-w-4xl mx-auto w-full">
        ...
        {/* List block */}
        <FadeUp delay={0.3} className="border-t border-black/18">
          {faqs.map((faq, index) => (
