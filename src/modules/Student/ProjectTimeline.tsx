  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm">
      ...
      <div className="relative">
        {/* Background line */}
        <div className="absolute top-4 left-4 right-4 h-0.5 bg-slate-200 dark:bg-slate-800 -translate-y-1/2 hidden sm:block" />

        {/* Highlighted active line */}
        <div className="absolute top-4 left-4 w-[50%] h-0.5 bg-blue-600 dark:bg-blue-500 -translate-y-1/2 hidden sm:block" />

        <div className="grid grid-cols-1 sm:grid-cols-5 gap-6 sm:gap-4 relative z-10">
          {steps.map((step, idx) => (
            <div key={idx} className="flex sm:flex-col items-center sm:text-center gap-3 sm:gap-0">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center sm:mb-3 shrink-0 transition-all duration-300 ${
                  step.status === "completed"
                    ? "bg-blue-600 text-white shadow-md shadow-blue-500/10"
                    : step.status === "active"
                    ? "bg-white dark:bg-slate-900 border-4 border-blue-605 text-blue-600 dark:text-blue-400 dark:border-blue-500 shadow-lg scale-105"
